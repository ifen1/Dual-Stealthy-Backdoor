...
class InversionEngine(nn.Module):
    def __init__(self, opt, init_mask, init_pattern):
        ...
        self.mask_tanh_input = nn.Parameter(init_mask_input)
        self.pattern_tanh_input = nn.Parameter(init_pattern_input)
        ...
        if "wanet" in opt.model_path:
            ...
            grid_temps = (identity_grid + 0.5 * noise_grid / 32) * 1.0
            opt.grid_temps = grid_temps.to(opt.device)
    ...
    def _get_classifier(self, opt):
        if opt.arch:
            ...
            if opt.arch == "resnet18":
                # classifier = getattr(models, 'resnet18')(num_classes=10).to(opt.device)
                classifier = resnet.resnet18(num_classes=opt.num_classes).to(opt.device)
            if opt.arch == "nin":
...
def train(opt, init_mask, init_pattern):
    ...
    for epoch in range(opt.epoch):
        ...
        if (epoch+1)%1==0:
            ...
            torchvision.utils.save_image(inversion_engine.example_ae_img[0], "./encoder_decoder_re_imgs/bad"+str(opt.target_label)+".jpg", normalize=True)
            torchvision.utils.save_image(inversion_engine.example_ori_img[0], "./encoder_decoder_re_imgs/ori_bad"+str(opt.target_label)+".jpg", normalize=True)
            # print("inversion_engine.example_space_trigger.max():",inversion_engine.example_space_trigger.max())
        if early_stop:
            break

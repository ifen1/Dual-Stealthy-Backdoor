...
parser.add_argument('--poison-type', type=str, default='wanet', choices=['badnets', 'blend', 'clean-label', 'benign','DUBA','wanet'],
...
def main():
    ...
    if args.poison_type in ['badnets', 'blend']:
        poison_train, trigger_info = poison.add_trigger_cifar(data_set=clean_train, trigger_type=trigger_type, poison_rate=args.poison_rate,
        ...
        trigger_info = None
    elif args.poison_type == 'DUBA':
        poison_train, trigger_info = poison.add_DUBA_trigger(data_set=clean_train,poison_rate=args.poison_rate,poison_target=args.poison_target)
        poison_test = poison.add_DUBA_test(data_set = clean_test,poison_target=args.poison_target,exclude_target=True)
    elif args.poison_type == 'wanet':
        poison_train, trigger_info = poison.add_wanet_trigger(data_set=clean_train,poison_rate=args.poison_rate,poison_target=args.poison_target)
        poison_test = poison.add_wanet_test(data_set = clean_test,poison_target=args.poison_target,exclude_target=True)
    else:
    ...
    # model_path = 'logs/models/model_last.th'
